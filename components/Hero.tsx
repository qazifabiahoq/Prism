...
export function Hero({
  ...
  return (
    <section className="relative overflow-hidden bg-white pb-24 pt-14 sm:pb-32 sm:pt-20">
      {/* Background texture */}
      <div className="pointer-events-none absolute inset-0 bg-grid-pattern bg-[size:44px_44px] opacity-60 [mask-image:radial-gradient(ellipse_70%_60%_at_50%_0%,black_10%,transparent_75%)]" />
      ...
      <div className="container-shell relative">
        <div className="mx-auto flex max-w-3xl flex-col items-center text-center">
          ...
          >
            <button
              onClick={onUpload}
              className="focus-ring group flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-accent-500 to-accent-600 px-7 py-3.5 text-[0.95rem] font-bold text-white shadow-glow transition hover:brightness-110 active:scale-[0.98] sm:w-auto"
            >
            ...
            <button
              onClick={onTryDemo}
              className="focus-ring w-full rounded-full border border-slate-200 bg-white px-7 py-3.5 text-[0.95rem] font-semibold text-ink-800 transition hover:border-slate-300 hover:bg-surface-50 sm:w-auto"
            >
          ...
          <div
            className="animate-fade-up mt-10 flex flex-wrap items-center justify-center gap-x-7 gap-y-3 text-xs font-medium text-slate-500 sm:text-sm"
            style={{ animationDelay: "0.3s" }}
          >
            <span className="inline-flex items-center gap-1.5">
              <ShieldCheck size={15} className="text-accent-600" /> No bank linking required
            </span>
            <span className="inline-flex items-center gap-1.5">
              <Zap size={15} className="text-accent-600" /> Results in ~30 seconds
            </span>
            <span className="inline-flex items-center gap-1.5">
              <TrendingUp size={15} className="text-accent-600" /> 87% forecast accuracy
            </span>
          </div>
        </div>
        ...
        >
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_30px_80px_-24px_rgba(19,24,38,0.22)] sm:rounded-3xl sm:p-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: "Wellness Score", value: "78", accent: "text-accent-600" },
                { label: "7-Day Forecast", value: "$412", accent: "text-ink-950" },
                { label: "Anomalies Found", value: "3", accent: "text-rose-500" },
                { label: "Forecast R²", value: "0.89", accent: "text-emerald-600" },
              ].map((m) => (
                <div
                  key={m.label}
                  className="rounded-xl border border-slate-200 bg-surface-50 p-4 text-left"
                >
            ...
            <div className="mt-3 h-28 w-full overflow-hidden rounded-xl border border-slate-200 bg-surface-50 sm:h-36">
              <svg viewBox="0 0 400 100" className="h-full w-full" preserveAspectRatio="none">
                <defs>
                  <linearGradient id="heroChart" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#5457E5" stopOpacity="0.28" />
